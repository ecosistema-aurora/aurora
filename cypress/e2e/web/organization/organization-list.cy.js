describe('Página de Listar de Organizações', () => {
    ...
    it('Garante que o filtro de ordenação funciona', () => {
        cy.get('#order-select').should('exist').should('be.visible').select('ASC'); // Seleciona "Mais Recente"
        cy.wait(1000);
        cy.get('.organization-name').first().invoke('text').then((firstOrgASC) => {
            cy.get('#order-select').select('DESC'); // Seleciona "Mais Antigo"
            cy.wait(1000);
            cy.get('.organization-name').first().invoke('text')
                .should((firstOrgDESC) => {
                    expect(firstOrgDESC).not.to.eq(firstOrgASC);
                });
        });
    });
